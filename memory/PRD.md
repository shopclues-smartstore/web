...
- [x] **Products Page with Progressive Sync Availability** (`/products`) — marketplace sync status banner with progress bar, marketplace filter dropdown with inline sync statuses (Synced/Syncing/Pending), **status filter dropdown** (All Statuses/Ready/Under review/Action required with color dots + live counts, combinable with marketplace filter), product table showing only synced marketplace products, skeleton placeholder states for non-synced marketplaces, toast notifications when sync completes (Meesho at 8s, Wish at 15s), sidebar "Syncing" badge, product actions (View/Edit/Publish toggle), search by name/SKU, "All synced" completion state. **Marketplace SVG logos** (Amazon, Flipkart, Meesho, Wish) replace text names in filter dropdown, product table badges (color-tinted), and skeleton labels.
